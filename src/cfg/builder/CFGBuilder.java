...
public class CFGBuilder {
	...
	private void initialize() throws NoSuchFileException{
		Wini ini = null;
		try {
			String filename=GInfo.v().confPrefix+"builder.ini";
		...
		if (ini == null) {
			...
			throw new NoSuchFileException("builder.ini");
		}
	    setDumpFilePath(GInfo.v().dumpFile+ini.get("thread dump","filepath",String.class));
			...
			for(ThreadInfo ti:threadInfoCollection){
				...
				for(Integer i=stackDepth-1;i>=0;i=i-1){
					...
					if(traceNode!=null && traceNode.getType().equals("CALL")){
						...
						if(callNode.getStmt().toString().contains(method.getSignature()))
							// Add an inter-procedural edge to the CFG of the current thread
					...
					for(Unit u:ug){
						...
						if(preds.size()==0){
							...
							if(u instanceof JInvokeStmt)
								...
								ti.getCfg().addEdge(enterNode, new NormalNode(u));
						}
					}
